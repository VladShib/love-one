...
if (startPepsaBtn) {
    startPepsaBtn.addEventListener('click', () => {
        startPepsaBtn.blur();
        // ВАЖНО: Гарантируем переключение экранов отображения
...
if (gameExitBtn) {
    gameExitBtn.addEventListener('click', (e) => {
        ...
        e.stopPropagation();
        stopGame();
        // Завершаем паркур, если он был активен
...
if (startParkourBtn) {
    startParkourBtn.addEventListener('click', () => {
        ...
        // ВОТ ЗДЕСЬ ИСПРАВЛЕНО: принудительно включаем canvas на экране!
        const pCanvas = document.getElementById('parkourCanvas');
        if(pCanvas) pCanvas.style.display = 'block';
        
        if(gameScreen) gameScreen.style.display = 'flex';
        
        // Запуск объекта из другого файла
...
// ==========================================
// ЗАПУСК МИНИ-ИГРЫ №5 «ОТФИГАЧИТЬ ШКОЛОТУ»
// ==========================================
...
if (startFightBtn) {
    startFightBtn.addEventListener('click', () => {
        ...
        // ГЛОБАЛЬНЫЙ ФЛАГ: Говорим остальным играм остановиться и не рисовать ничего на Canvas!
        window.isFightModeActive = true; 
        
        // Полностью прячем контейнеры других игр
        const gameArea = document.getElementById('gameArea');
        const pepsaScoreContainer = document.getElementById('pepsaScoreContainer'); 
        if (gameArea) gameArea.style.display = 'none';
        if (pepsaScoreContainer) pepsaScoreContainer.style.display = 'none';
        
        // Включаем холст файтинга
        const pCanvas = document.getElementById('parkourCanvas');
        const gameScreen = document.getElementById('gameScreen');
        if (pCanvas) pCanvas.style.display = 'block';
        if (gameScreen) gameScreen.style.display = 'flex';
