...
const BestwayParkour = {
    ...
    isMobile: false, // Флаг для проверки мобилки
    ...
    start(canvasId) {
        ...
        // Настройки баланса: мобилка vs ПК
        if (this.isMobile) {
            this.gameSpeed = 260;           // Меньше скорость — больше времени среагировать
            this.player.gravity = 1900;     // Плавное падение
            this.player.jumpForce = -610;   // Комфортная высота прыжка под низкую гравитацию
            this.player.flipSpeed = 5;      // Чуть медленнее крутит сальто
            this.player.x = 60;             // Сдвигаем игрока левее, чтобы видеть больше карты впереди
        } else {
            this.gameSpeed = 380;
        ...
        this.obstacles = [
            { x: 700, y: this.canvas.height - 245, size: 25 },
            { x: 1200, y: this.canvas.height - 195, size: 25 }
        ];
    ...
    resize() {
        if (this.canvas) {
            ...
            // Проверяем, мобилка ли это прямо сейчас
    ...
    update(dt) {
        ...
        if (this.buildings.length < 4) {
            ...
            // На мобилках делаем пропасти меньше, чтобы игрок гарантированно долетал
            let gap = this.isMobile 
                ? 80 + Math.random() * 80 
                : 110 + Math.random() * 120;
            ...
            if (width > 300 && Math.random() < 0.55) {
                this.obstacles.push({
                    x: lastB.x + lastB.w + gap + 100 + Math.random() * (width - 160),
    ...
    draw() {
        ...
        // На мобилках сдвигаем текст чуть правее, чтобы иконка крестика его не перекрывала
        ctx.fillText(`СЧЕТ ПАРКУРА: ${this.score}`, this.isMobile ? 140 : 120, 32);
    },
