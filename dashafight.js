// =================================================================
// --- СВЕРХТАКТИЧЕСКИЙ ФАЙТИНГ «DASHA FIGHT 1v1» (ВЕРСИЯ 2.3) ---
// =================================================================
...
const DashaFight = {
    ...
    // НАСТРОЙКИ ХП: Увеличили макс. ХП до 250 для затяжного тактического боя
    sasha: {
    ...
    start(canvasId) {
        ...
        this.sasha.state = 'idle'; this.sasha.attackCd = 0; this.sasha.blockCd = 0; this.sasha.slashAnim = null;
        ...
        this.dasha.state = 'idle'; this.dasha.attackCd = 0; this.dasha.blockCd = 0; this.dasha.slashAnim = null;
    ...
    resize() {
        ...
        this.touchControl.btnHit = { x: w - 170, y: h - 85, r: 38, active: false };
        this.touchControl.btnBlock = { x: w - 75, y: h - 120, r: 38, active: false };
    ...
    setupControls() {
        ...
        this.canvas.ontouchmove = (e) => {
            e.preventDefault();
            if (this.gameState !== 'playing' || !this.touchControl.stick.active) return;
    ...
    triggerHit(char) {
        if (char.attackCd <= 0 && char.state !== 'blocking' && char.state !== 'attacking') {
            char.state = 'attacking'; char.stateTimer = 0.35; char.attackCd = 0.9; // Слегка уменьшили КД для динамики
            char.slashAnim = { progress: 0, dir: char.side === 'left' ? 1 : -1, triggeredDamage: false };
        }
    },

    triggerBlock(char) {
        if (char.blockCd <= 0 && char.state !== 'attacking' && char.state !== 'blocking') {
        ...
        if (this.checkCollision(char.attackBox, target)) {
            ...
            if (target.state === 'blocking') {
                if (target.isParryWindow) {
                    this.audio.parry(); 
                    char.hp -= 15; // Урон атакующему от идеального парирования
                    ...
                    this.audio.block(); 
                    target.hp -= 1; // Урон сквозь обычный блок
                ...
                // РАЗДЕЛЬНЫЙ УРОН:
                if (char === this.dasha) {
                    target.hp -= 22; // БОСС-УРОН: Даша сносит 22 хп за удар (можешь поставить больше, например 25-30)
                } else {
                    target.hp -= 12; // Саша оставляет базовые 12 урона
                }
    ...
    updateAI(dt) {
        let ai = this.dasha; let p = this.sasha; ai.aiTimer -= dt;
        ...
        // НАКАЗАНИЕ ЗА СПАМ ВПРИТЫК: Если игрок подошел вплотную (< 85px) и атакует, Даша мгновенно контратакует или ставит парри
        if (dist < 85 && p.state === 'attacking') {
            if (ai.blockCd <= 0 && Math.random() < 0.6) {
                this.triggerBlock(ai); // Ставит щит-парирование, нанося Саше урон обратно
                return;
            } else if (ai.attackCd <= 0) {
                this.triggerHit(ai); // Мгновенный размен ударами
                return;
            }
        }

        // ДИНАМИКА: Даша думает быстрее (интервал от 0.15 до 0.4 сек) и атакует НАМНОГО ЧАЩЕ
        if (ai.aiTimer <= 0) {
            ai.aiTimer = 0.15 + Math.random() * 0.25;

            // Если Даша близко, шанс атаки резко увеличен
            if (dist < 120) {
                let r = Math.random();
                if (r < 0.65 && ai.attackCd <= 0) { 
                    ai.aiState = 'attack'; // 65% шанс сразу всечь
                ...
                ai.aiState = 'approach'; // Если далеко — агрессивно сокращает дистанцию
            } else {
        ...
        if (ai.aiState === 'attack') {
            ...
            ai.x += (ai.x > p.x) ? -160 * dt : 160 * dt; // Скорость сближения повышена до 160
    ...
    loop(now) {
        // Если игра окончена, ВООБЩЕ ничего не делаем внутри цикла и выходим
    ...
    endFight() {
        // 1. Мгновенно меняем статус, останавливая игровой цикл движка
        this.gameState = 'gameover'; 
        
        // 2. Глушим анимацию, чтобы не было фантомных кадров
        ...
        // Тексты финала
        let resultText = this.sasha.hp > 0 ? "Ахахахаха туда школоту!" : "Изи, мелкая ботиха";
        let textColor = this.sasha.hp > 0 ? "#00ffcc" : "#ff4b6e";

        // 3. Запускаем отрисовку экранаGameOver с задержкой в 50 миллисекунд,
        // чтобы браузер точно успел завершить все предыдущие процессы рисования арены
        setTimeout(() => {
            this.drawGameOver(resultText, textColor);
        }, 50);

        // 4. Закрываем экран через 4 секунды и возвращаем на главный сайт
        setTimeout(() => {
